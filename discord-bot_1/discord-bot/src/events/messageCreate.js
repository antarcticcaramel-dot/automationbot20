...
module.exports = {
  ...
  async execute(message, client) {
    ...
    if (channelName.includes('art')) {
      ...
      // Keep the original message, just add like/dislike buttons below it
      ...
      await message.channel.send({
        content: `> 🎨 Artwork by ${message.author} — vote below!`,
        components: [row]
      });
      return;
    }
    ...
    if (channelName.includes('bot-command') || channelName.includes('general')) {
      const lower = message.content.toLowerCase();
      if (lower.includes('help') || lower.includes('support') || lower.includes('how do i') || lower.includes('issue')) {
        await message.reply({
          embeds: [new EmbedBuilder()
            .setColor(0xA855F7)
            .setTitle('🆘 Need Help?')
            .setDescription([
              ...
              '',
              `💬 **Discord Server:** [Join here](${DISCORD_INVITE})`,
              `🎮 **Roblox Group:** [Join here](${ROBLOX_GROUP})`,
              '',
              'Our staff team is always happy to help! 💜'
            ].join('\n'))
            .setFooter({ text: 'Join the community for the fastest support!' })
          ]
        });
      }
      return;
